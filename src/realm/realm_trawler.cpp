...
public:
    ...
    std::string get_table_name(unsigned i) const
    {
...
void Table::print_columns(const Group& group) const
{
    for (unsigned i = 0; i < m_column_names.size(); i++) {
        ...
        if (type == realm::col_type_Link || type == realm::col_type_LinkList) {
            auto target_table = m_column_subspecs.get_val(get_subspec_ndx_after(i));
            attributes = " " + group.get_table_name(target_table);
        }
        else {
        ...
        std::cout << "        " << m_column_names.get_string(i) << ": " << type_to_string(type) << attributes
                  << std::endl;
    }
}
